...
elif [[ -d "${NUMT_REPO}" && -f "${NUMT_REPO}/run_numt_end2end.sh" ]]; then
...
CONFIG
    ...
    log "WARNING: NUMT_REPO is unavailable or missing run_numt_end2end.sh: ${NUMT_REPO}"
    log "WARNING: Creating an empty high-confidence NUMT BED so round 1 can continue with a chrM-only decoy"
    : > "${NUMT_BED}"
fi
